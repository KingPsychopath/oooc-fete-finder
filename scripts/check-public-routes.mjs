...
const checkPublicHtml = async () => {
	...
	const eventDetails = await fetchText(eventDetailsPath);
	if (!eventDetails.ok) {
		fail(`event details endpoint returned HTTP ${eventDetails.status}`);
	}
	assertIncludes(
		eventDetails.text,
		'"event"',
		"event details endpoint returns one selected event",
	);
	assertIncludes(
		eventDetails.text,
		"Krispy Jam N°29 - Tascha",
		"event details endpoint includes selected event title",
	);
