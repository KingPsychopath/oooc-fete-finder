...
const makeEvent = (overrides: Partial<Event> & { eventKey: string; date: string }): Event => ({
	slug: overrides.eventKey,
	id: overrides.eventKey,
	name: overrides.eventKey,
	day: "friday",
	date: overrides.date,
	arrondissement: 11,
	link: "https://example.com",
	type: getEventTypeForDate(overrides.date),
	genre: ["Afrobeats"],
	venueTypes: ["indoor"],
	indoor: true,
	verified: true,
...
describe("createRegularEventsComparator", () => {
	...
	it("uses deterministic tie-breakers when date/time matches", () => {
		const now = new Date("2026-06-21T12:00:00");
		const events: Event[] = [
			makeEvent({ eventKey: "zeta", date: "2026-06-22", time: "11:00", name: "Zulu" }),
			makeEvent({ eventKey: "alpha", date: "2026-06-22", time: "11:00", name: "Alpha" }),
		];
