...
export async function GET(request: NextRequest) {
  try {
    ...
    const hasCustomParams = searchParams.has('title') || 
    ...
    if (!hasCustomParams) {
      ...
      for (const imagePath of defaultImagePaths) {
        try {
          ...
          console.log(`📝 Default image not found at ${imagePath} - continuing to dynamic generation`);
          if (error instanceof Error && error.message.includes('Unsupported image type')) {
            console.log(`💡 Note: "Unsupported image type: unknown" means the image file doesn't exist at this path`);
          }
      ...
      console.log(`🎨 No static default images found - generating dynamic OG:image`);
    }
    
    // Extract and sanitize parameters
    const title = sanitizeText(searchParams.get('title') || 'Fête Finder');
    const subtitle = sanitizeText(searchParams.get('subtitle') || 'Interactive Paris Music Events Map');
    const themeParam = searchParams.get('theme') || 'default';
    const theme = ALLOWED_THEMES.includes(themeParam as typeof ALLOWED_THEMES[number]) ? themeParam : 'default';
    const eventCount = Math.min(parseInt(searchParams.get('eventCount') || '0') || 0, 9999);
    ...
    const localImage = localImageParam && localImageParam.startsWith('/og-images/') 
      ? localImageParam 
      : null;
    ...
    const useDefaultImage = !localImage && !searchParams.has('title') && !searchParams.has('theme');
    const finalImage = localImage || (envDefaultImage && useDefaultImage ? envDefaultImage : (useDefaultImage ? defaultImagePath : null));
    ...
    if (error instanceof Error) {
      if (error.message.includes('Unsupported image type')) {
        console.error('💡 Image Error: The specified image file could not be loaded. This usually means:');
        console.error('   - The image file does not exist at the specified path');
        console.error('   - The image format is not supported (use PNG, JPEG, or WebP)');
