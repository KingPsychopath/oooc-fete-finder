...
export const env = createEnv({
  ...
  server: {
    ...
    CACHE_EXTENSION_DURATION_MS: z.coerce.number().int().min(1).default(7200000), // 2 hours

    // Memory management
    CACHE_MAX_MEMORY_BYTES: z.coerce.number().int().min(1).default(52428800), // 50MB
    CACHE_MEMORY_CHECK_INTERVAL_MS: z.coerce.number().int().min(1).default(300000), // 5 mins
    ...
    CACHE_METRICS_RESET_INTERVAL_MS: z.coerce.number().int().min(1).default(86400000), // 24 hours
    CACHE_DEDUPLICATION_TIMEOUT_MS: z.coerce.number().int().min(1).default(5000), // 5 seconds
    ...
    CACHE_BOOTSTRAP_MODE: z.enum(["strict", "fallback", "graceful"]).default("graceful"),
...
export const getGoogleSheetsStatus = () => ({
  ...
  hasDirectUrl: !!(env.GOOGLE_SHEETS_URL && env.GOOGLE_SHEETS_URL.startsWith('https://')),
  hasRemoteUrl: !!(env.REMOTE_CSV_URL && env.REMOTE_CSV_URL.startsWith('https://')),
