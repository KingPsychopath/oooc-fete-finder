...
export class CacheMemoryManager {
	 ...
	 */
	static checkMemoryLimits(currentUsage: number, eventCount: number = 0, lastMemoryCheck: number = 0): MemoryLimitsCheck {
		const now = Date.now();
		...
		const shouldLog = (now - lastMemoryCheck) > MEMORY_CONFIG.MEMORY_CHECK_INTERVAL;
	...
	static validateMemoryForUpdate(
		...
		if (memoryCheck.needsEmergencyCleanup) {
			...
			const recheckMemory = this.checkMemoryLimits(newMemoryUsage, newData.length);
	...
	static getMemoryConfig() {
		return {
			...
			cleanupThresholdMB: ((MEMORY_CONFIG.MAX_MEMORY_USAGE * MEMORY_CONFIG.CLEANUP_THRESHOLD) / 1024 / 1024).toFixed(1),
			emergencyThresholdMB: ((MEMORY_CONFIG.MAX_MEMORY_USAGE * MEMORY_CONFIG.EMERGENCY_THRESHOLD) / 1024 / 1024).toFixed(1),
		};
	}
	 ...
	 */
	static getMemoryHealth(currentUsage: number, _eventCount: number = 0): {
		status: "healthy" | "warning" | "critical";
		message: string;
		utilizationPercent: number;
	} {
		const utilizationPercent = (currentUsage / MEMORY_CONFIG.MAX_MEMORY_USAGE) * 100;
