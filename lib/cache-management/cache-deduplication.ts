...
export class CacheRequestDeduplicator {
	 ...
	 */
	static async deduplicateGetEvents<T>(
	 ...
	 */
	static async deduplicateForceRefresh<T>(
		operation: () => Promise<T>,
	): Promise<T> {
	 ...
	 */
	static async deduplicateFullRevalidation<T>(
		path: string,
		operation: () => Promise<T>,
	): Promise<T> {
	 ...
	 */
	static async deduplicateCacheStatus<T>(
		operation: () => Promise<T>,
	): Promise<T> {
	...
	static getDeduplicationStatus() {
		...
		return {
			...
			hasFullRevalidation: pendingKeys.some((key) =>
				key.startsWith("fullRevalidation:"),
			),
