...
export function generateICSContent(event: Event): string {
  ...
  const location = event.location && event.location !== "TBA" 
    ? `${event.location}, ${event.arrondissement}e Arrondissement, Paris, France`.replace(/[,;\\]/g, "\\$&")
...
function createEventDescription(event: Event): string {
  ...
  // Add link
  const primaryLink = event.links && event.links.length > 0 ? event.links[0] : event.link;
...
export function downloadICSFile(event: Event): void {
  ...
  const fileName = `${event.name.replace(/[^a-zA-Z0-9\s]/g, "").replace(/\s+/g, "-").toLowerCase()}-${event.date}.ics`;
...
export function addToCalendar(event: Event): void {
  try {
    ...
    if (isMobile) {
      ...
      const link = document.createElement("a");
      link.href = dataUri;
      link.download = `${event.name.replace(/[^a-zA-Z0-9\s]/g, "").replace(/\s+/g, "-").toLowerCase()}-${event.date}.ics`;
