...
type Setup = {
	DataManager: typeof import("@/lib/data-management/data-manager").DataManager;
	localEventStore: {
...
const loadDataManager = async (mode: "remote" | "local" | "test" = "remote"): Promise<Setup> => {
	...
	vi.doMock("@/lib/data-management/local-event-store", () => ({
		LocalEventStore: localEventStore,
	}));

	vi.doMock("@/lib/data-management/data-processor", () => ({
		processCSVData,
		isValidEventsData,
	}));

	vi.doMock("@/lib/data-management/csv/fetcher", () => ({
		fetchLocalCSV,
	}));

	const { DataManager } = await import("@/lib/data-management/data-manager");
