...
const TICKET_ACTIVITY_MAX_VISIBLE = 9;
const TICKET_ACTIVITY_FRESH_WINDOW_HOURS = 6;
const MS_PER_HOUR = 60 * 60 * 1000;

const getTicketActivityTotal = (event: Event): number =>
	(event.ticketExchangeSellingCount ?? 0) +
	(event.ticketExchangeLookingCount ?? 0);

const isTicketActivityFresh = (
	...
	return (
		...
		TICKET_ACTIVITY_FRESH_WINDOW_HOURS * MS_PER_HOUR
	);
};

const getTicketActivityDisplayModesForEvents = (
	events: Event[],
	now: Date = new Date(),
	...
	const eligibleEvents = events
		...
		.slice(0, TICKET_ACTIVITY_MAX_VISIBLE);
...
export function EventsSearchFiltersProvider({
	...
	const ticketActivityDisplayModes = useMemo(
		() => getTicketActivityDisplayModesForEvents(spotlightEligibleEvents),
		[spotlightEligibleEvents],
	);
