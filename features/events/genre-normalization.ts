...
const rawGenreAliases: Array<[string, MusicGenre]> = [
	["amapiano", "amapiano"],
	["afrobeats", "afrobeats"],
...
const genreAliasMap = new Map<string, MusicGenre>(
	rawGenreAliases.map(([alias, genre]) => [normalizeSearchText(alias), genre]),
);
...
export const resolveMusicGenre = (value: string): MusicGenre | null => {
	const normalized = normalizeSearchText(value);
	if (normalized.length === 0) return null;
	return genreAliasMap.get(normalized) ?? null;
};
